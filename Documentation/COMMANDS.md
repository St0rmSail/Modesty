...
Opening, resuming, or reopening a bookmark also opens the large **Reading Desk**. Its Previous and Next Passage controls revisit passages shown during that Desk session or request the next bounded passage. Save Place, Bookmark, and Bookmark with Note reuse the same exact-edition rules above. When an earlier cached passage is being reviewed, those three writing controls are disabled because the current backend endpoint is newer. Closing the Desk returns to chat without saving progress automatically.
