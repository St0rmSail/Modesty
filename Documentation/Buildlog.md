...
Routine Librarian work now accepts a deliberately small contextual vocabulary while translating every request into the existing deterministic operations. Drew can review duplicates, choose a displayed folder/title copy, approve the single prepared exact-duplicate action, open a chapter, continue, save a place, and resume after restart without typing hashes or job identifiers. Vague copy descriptions ask for distinction, contextual approval exists only while one duplicate plan is pending in the current Study session, and reading progress still changes only after explicit place saving.
...
Build 0.35.0 - The Angler Read-only Adapter Review (implemented; live acceptance pending)

The first adapter recognises The Angler's local Avalanche ADF binary containers, records bounded size and SHA-256 identity in memory, detects matching recovery snapshots and refuses path escape, symlinks, oversized files, or a file changing during inspection. It deliberately does not decode internal fields or claim level, cash, inventory, unlocks, or catches because no authoritative field schema has yet been established. The exact command is documented and focused automated tests pass; live Modesty acceptance remains required.

The live before/after observation subsequently proved that an ordinary play-and-save cycle rewrites almost every active and recovery container. A content-free 4 KiB block comparison was added, but The Angler reverse-engineering path is parked before speculative field decoding.

Build 0.35 then moved to the practical RF4 evidence path. A bounded RF4 Tackle Box importer now extracts its 251-species embedded catalogue, validates a fixed record shape and five named species, records the exact RF4 community source and bundle hash, and writes transactionally only to the RF4 codex at the weakest evidence rank. The first live import stored 251 species references, 133 distinct baits, 1,159 unique species-bait links, 18 waters and 580 species-water links. It rejected a `9,999,999 g` apparent sentinel as unknown. No hotspots, recipes, personal progress or other games were changed. Dynamic hotspots are canonically rumoured, known after local verification, and favourite only after Drew explicitly chooses them. All 161 automated tests passed.

Drew repeated the RF4 Almanac import through Modesty's Study and accepted the exact source-labelled report on 2026-09-06. Build 0.35 is complete at this restore point. Build 0.36 will expose concise, natural RF4 species questions over the imported evidence without yet adding live hotspots or personal game-state monitoring.

Build 0.36.0 - RF4 Codex Questions (complete)

The Fishing Buddy now answers short natural one-species RF4 questions from the private source-ranked catalogue. The deterministic path covers bait suggestions, named-bait checks, listed waters, trophy thresholds and species overviews; bounds long lists; preserves unknown fields; tolerates a clear minor spelling error; refuses uncertain species; and cannot leak another game's records. Absence from the community bait list is explicitly not treated as proof that a bait cannot work.

Drew accepted the complete live Study question set on 2026-09-06, including tench, common roach, bream, Atlantic cod, `comman roch`, and a nonexistent species. All 165 automated tests and syntax checks passed. The Fishing Buddy reaches demonstrated Level 2 bounded retrieval. Dynamic hotspots, recipes, personal progress, automatic catches and background refresh remain outside this build.
