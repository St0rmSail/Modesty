...
Step Three adds the first real provider without granting general web access. Online mode is distinct from loopback mode and cannot be switched while the Gateway is open. The provider accepts only the approved question, `Research Kathleen McNulty and the first ENIAC programmers`, previews that exact outbound question, sends no Bookshelf passages, requests at most five Open Access records, retains source URLs and bounded metadata, and quarantines the return as unverified in the Bookshelf Inbox. Empty, malformed, excessive, unauthenticated, or unrelated results fail without creating a return. Live expedition demonstration remains pending.
